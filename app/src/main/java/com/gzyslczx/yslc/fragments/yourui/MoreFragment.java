...
public class MoreFragment extends BaseFragment<DailyStockFragmentBinding> implements OnDailyLongPressListener, OnDailyStockLoadMoreListener,
        View.OnClickListener, OnItemClickListener {

    private final String TAG = "MoreStockFrag";

    public static final String MoreType = "MT";
    ...
    public void onLoadMoreDailyStock() {
        if (mViewBinding.dailyStockLoadMore.getVisibility() == View.GONE) {
            Log.d(getClass().getSimpleName(), "K线加载更多");
    ...
    * */
    ...
    public void onMoreTypeStockEvent(MoreTypeStockEvent event){
        OFFSET = -1;
        PERIOD = event.getType();
        if (event.getRemit()!=99){
            REMIT = event.getRemit();
        }
        EventBus.getDefault().post(new NoticeDailyKLineLoadMoreEvent(OFFSET, PERIOD, REMIT));
    }
